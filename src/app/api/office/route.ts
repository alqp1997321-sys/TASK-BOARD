...
export async function GET() {
  try {
    ...
    const officeFile = gist.files["office.json"];
    if (!officeFile) return NextResponse.json([]);
    const data = JSON.parse(officeFile.content);
...
export async function POST(request: Request) {
  try {
    const body: OfficeAgent[] = await request.json();
    const gist = await getGist();
    const existingFiles: Record<string, string> = {};
    for (const [name, file] of Object.entries(gist.files as Record<string, { content: string }>)) {
      if (name === "office.json") continue;
      existingFiles[name] = file.content;
    }
    existingFiles["office.json"] = JSON.stringify(body, null, 2);
