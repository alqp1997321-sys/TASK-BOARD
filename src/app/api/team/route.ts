...
export async function GET() {
  try {
    ...
    const teamFile = gist.files["team.json"];
    if (!teamFile) return NextResponse.json([]);
    const data = JSON.parse(teamFile.content);
...
export async function POST(request: Request) {
  try {
    ...
    for (const [name, file] of Object.entries(gist.files as Record<string, { content: string }>)) {
      if (name === "team.json") continue;
      existingFiles[name] = file.content;
    }
    existingFiles["team.json"] = JSON.stringify(body, null, 2);
